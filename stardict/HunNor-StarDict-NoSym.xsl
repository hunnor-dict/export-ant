...
<xsl:stylesheet version="2.0" xmlns:dict="http://dict.hunnor.net" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" exclude-result-prefixes="dict">
	...
	<xsl:template match="dict:entry">
		...
		<xsl:apply-templates select="dict:formGrp" mode="noSym"/>
	</xsl:template>
	...
	<xsl:template match="dict:formGrp" mode="noSym">
		<!-- TODO: make an article of each non-primary and inflected form -->
		<xsl:variable name="pointTo" select="dict:form[@primary='yes']/dict:orth"/>
		<xsl:for-each select="distinct-values(dict:form[@primary='no']/dict:orth|dict:form/dict:inflPar/dict:inflSeq)">
			<xsl:if test="not(.=$pointTo)">
				<article>
					<key>
						<xsl:value-of select="."/>
					</key>
